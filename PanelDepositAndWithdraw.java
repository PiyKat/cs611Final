...
public class PanelDepositAndWithdraw {
    ...
    public PanelDepositAndWithdraw(String userId) {
        this.userId = userId;
        ...
        JLabel label1 = new JLabel("Transfer Type : ");
        JRadioButton deposit = new JRadioButton("Deposit");
        JRadioButton withdraw = new JRadioButton("Withdrawal");
        ButtonGroup btnGrouploan = new ButtonGroup();
        btnGrouploan.add(deposit);
        btnGrouploan.add(withdraw);
        deposit.setSelected(true);
        rowBox1.add(label1);
        rowBox1.add(deposit);
        rowBox1.add(withdraw);

        Box rowBox2 = Box.createHorizontalBox();
        JLabel label2 = new JLabel("Select the account: ");
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    ...
                    if (deposit.isSelected()) {
                        if (bec.depositMoney(userId, String.valueOf(accountList.getSelectedValue()), amount)) {
                        ...
                        if (bec.withdrawMoney(userId, String.valueOf(accountList.getSelectedValue()), amount)) {
                            succ.setVisible(true);
                            try {
                                Thread.sleep(5000);
                            } catch (Exception e1) {
